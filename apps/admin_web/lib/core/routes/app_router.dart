...
final appRouterProvider = Provider<GoRouter>((ref) {
	return GoRouter(
		...
		routes: [
			GoRoute(path: '/teken_in', name: 'teken_in', builder: _builder(const TekenInPage())),
			GoRoute(path: '/registreer_admin', name: 'registreer_admin', builder: _builder(const RegistreerAdminPage())),
			GoRoute(path: '/wag_goedkeuring', name: 'wag_goedkeuring', builder: _builder(const WagVirGoedkeuringPage())),
			GoRoute(path: '/dashboard', name: 'dashboard', builder: _builder(const DashboardPage())),
			GoRoute(path: '/spyskaart', name: 'spyskaart', builder: _builder(const SpyskaartBestuurPage())),
			GoRoute(path: '/week_spyskaart', name: 'week_spyskaart', builder: _builder(const WeekSpyskaartPage())),
			GoRoute(path: '/templates/kositem', name: 'templates_kositem', builder: _builder(const KositemTemplaatPage())),
			GoRoute(path: '/templates/week', name: 'templates_week', builder: _builder(const WeekTemplaatPage())),
			GoRoute(path: '/bestellings', name: 'bestellings', builder: _builder(const BestellingBestuurPage())),
			GoRoute(path: '/gebruikers', name: 'gebruikers', builder: _builder(const GebruikersBestuurPage())),
			GoRoute(path: '/kennisgewings', name: 'kennisgewings', builder: _builder(const KennisgewingsPage())),
			GoRoute(path: '/verslae', name: 'verslae', builder: _builder(const VerslaePage())),
			GoRoute(path: '/instellings', name: 'instellings', builder: _builder(const InstellingsPage())),
			GoRoute(path: '/hulp', name: 'hulp', builder: _builder(const HulpPage())),
			GoRoute(path: '/profiel', name: 'profiel', builder: _builder(const ProfielPage())),
		],
	);
});

GoRouterWidgetBuilder _builder(Widget child) => (BuildContext context, GoRouterState state) => PageScaffold(title: state.name ?? '', child: child); 